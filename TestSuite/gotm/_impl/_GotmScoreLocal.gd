...
static func _fetch_by_score_sort(params: Dictionary) -> Array:
	var matches := []
...
static func _fetch_counts(params: Dictionary) -> Array:
...
static func _fetch_rank(params: Dictionary) -> int:
...
static func _get_range_from_period(period: String) -> Array:
	...
	if day:
		...
		day = 1
	if granularity:
		var start = 1000 * Time.get_unix_time_from_datetime_dict({"year": year, "month": month, "day": day, "hour": 0, "minute": 0, "second": 0})
		var end_datetime = {"year": year, "month": month, "day": day, "hour": 0, "minute": 0, "second": 0}
		end_datetime[granularity] += 1
		var end = 1000 * Time.get_unix_time_from_datetime_dict(end_datetime) - 1
